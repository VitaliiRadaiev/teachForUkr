...
import { getSectionsPaddingClasses, getSectionsMarginClasses, getUrlToStaticImages, debounce, mergeRefs, buildApiPath } from "../../utils/utils";
...
export default function Edit({ attributes, setAttributes, clientId }) {
	...
	const fetchPosts = () => apiFetch({ path: `site-core/v1/review${selectedCategories.length ? `?category=${selectedCategories.join(',')}` : ''}` });
	const { ref, data, isLoading, refetch } = useFetchOnVisible(fetchPosts);

	const fetchPostsByIds = () => apiFetch({ path: `site-core/v1/review-by-ids?ids=${selectedPosts.join(',')}` });
	...
	const fetchCategoriesData = () => apiFetch({ path: 'site-core/v1/review-categories' });
	...
	const fetchPostsForFilter = () => {
		const path = buildApiPath('site-core/v1/review', {
	...
	const handleCategoryChange = (checkedCategory, selectedCategories, isChecked) => {
		...
		refetch(() => apiFetch({ path: `site-core/v1/review${ids.length ? `?category=${ids.join(',')}` : ''}` }));
	...
	useEffect(() => {
		refetchPostsByIds(() => apiFetch({ path: `site-core/v1/review-by-ids?ids=${selectedPosts.join(',')}` }));
	}, [selectedPosts]);
	...
	return (
		<>
				...
				<PanelBody title="Відобразити тільки по категоріях" initialOpen={true}>
					<div className={clsx({
						...
						<div className="mt-[20px] text-[14px] font-bold">
							Відобразити тільки обрані відгуки
						</div>
						<input
			...
			<section {...blockProps}>
				...
				<div ref={mergeRefs(ref, refCategories, postsByIdsRef, postsForFilterRef)}
					className={clsx(
					...
					{!!selectedPosts.length && !!renderPosts.length &&
						<div className={clsx(
							...
							<div className="text-lg">
								Обрані відгуки, <strong className="text-sm">карточки нижче відображаються тільки тут, в режимі наповення сайту</strong>
							</div>
							<div className="mt-[20px] selectedNews grid lg:grid-cols-3 gap-[10px] lg:gap-[24px]">
								{renderPosts.map(post => (
									<div className="swiper-slide !h-auto relative">
										<button
											onClick={() => {
												setAttributes({ selectedPosts: selectedPosts.filter(id => id != post.id) })
											}}
										...
										<div className="relative z-1flex flex-col h-full nested-bg-item rounded-[12px] p-[16px] md:p-[30px] pointer-events-none">
